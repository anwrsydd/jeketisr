...
async function get_history_live(start: number, end: number): Promise<JKT48.LastLive[]> {
    const q = query(collection(db, "jkt48_live_history"), orderBy("d_id"), startAfter(start), endAt(end));
...
async function get_member_live(sign_name: string): Promise<JKT48.LastLive[]> {
    ...
    const q = query(collection(db, "jkt48_live_history"), where("name", ">=", name), where("name", "<", end));
    const snapshot = await getDocs(q);
    const data = snapshot.docs.map((doc) => doc.data() as JKT48.LastLive).sort((a, b) => b.live_at - a.live_at);
    return data;
}

async function get_live_detail(id: number): Promise<JKT48.LastLive[]> {
    const q = query(collection(db, "jkt48_live_history"), where("d_id", "==", id));
    const snapshot = await getDocs(q);
    const data = snapshot.docs.map((doc) => doc.data() as JKT48.LastLive);
    return data;
}
