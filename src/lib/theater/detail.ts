...
async function get_theater_detail(id: number): Promise<API.TheaterDetail> {
    ...
    if (d?.error) {
        ...
        for (let i = 0; i < theater?.member_perform.length; i++) {
            const find_member = members.find(
                (datas) => datas.full_name === theater.member_perform[i],
            );
        ...
        const setlist_img_r = setlist_image.find((objk) =>
            objk.title.startsWith(d.title.split(" -")[0]),
        );
        const description = d.description !== undefined ? d.description : "";
        const setlist_img =
            setlist_img_r !== undefined ? setlist_img_r.url : d.image;
        return {
            image: d.image,
