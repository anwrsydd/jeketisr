...
type Data = { shows: API.TheaterDetail[]; error: boolean };
...
async function getData(protocol: string | null, host: string | null): Promise<Data> {
    try {
        const shows_r = await get_premium_live();
        const shows: API.TheaterDetail[] = [];
        for (let i = 0; i < shows_r.length; i++) {
            const dt_r = await fetch(protocol + "://" + host + "/api/theater/detail?id=" + shows_r[i].paid_live_id);
