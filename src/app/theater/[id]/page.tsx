...
export default function TheaterDetailPage({ params }: { params: { id: string | number } }) {
    const [detail, setDetail] = useState<any>();
    const [isError, setIsError] = useState<boolean>(false);
    const [isLoad, setIsLoad] = useState<boolean>(true);
    useEffect(() => {
        fetch("/api/theater/detail?id=" + params.id)
            .then((res) => res.json())
            .then((dt) => {
                try {
                    if (dt.error) {
                                    ...
                                    >
                                        <Link href={`/member/${o.sr_room_url_key?.toLowerCase()}`}>
                                            <Image
                                                ...
                                                alt={o.full_name}
                                            />
                                            <h4 className="text-sm font-medium">{o.full_name}</h4>
                                        </Link>
                                    </div>
                                ))}
                            </div>
